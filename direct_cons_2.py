...
def callback(ch, method, properties, body):
    print(f" A [x] {method.routing_key}:{body}")

def callbackB(ch, method, properties, body):
    print(f" B [x] {method.routing_key}:{body}")
    
def callbackC(ch, method, properties, body):
    print(f" C [x] {method.routing_key}:{body}")
...
if __name__=="__main__":
    ...
    # routing_key = "houseA"
    channel.queue_bind(
        exchange='direct2_logs', queue='PersonA_house')
    # channel.queue_bind(
    #     exchange='direct2_logs', queue='PersonA_house', routing_key='PersonB_house')
    # channel.queue_bind(
    #     exchange='direct2_logs', queue='PersonB_house', routing_key='PersonA_house')
    # channel.queue_bind(
    ...
    channel.basic_consume(
        queue='PersonA_house', on_message_callback=callbackB, auto_ack=True)
    
    channel.basic_consume(
        queue='PersonA_house', on_message_callback=callbackC, auto_ack=True)
